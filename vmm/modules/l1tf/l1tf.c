...
static void derive_cpu_masks(void)
{
	if (cpuid_leaf_0b_support()) {
		...
		const uint32_t full_mask = ~(uint32_t)0U;
		cpuid_params_t leaf_0b = {0xb, 0, 0, 0};
		asm_cpuid(&leaf_0b);

		mask_smt_shift = leaf_0b.eax & 0x1FU;
		smt_mask = ~(full_mask << mask_smt_shift);
		...
		core_plus_smt_mask = ~(full_mask << mask_core_shift);

		core_mask = core_plus_smt_mask ^ smt_mask;
		package_mask = full_mask << mask_core_shift;
	} else {
